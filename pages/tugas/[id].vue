<template>
  <div>
    <div class="font-bold text-4xl mx-12 my-6">Update Tugas</div>
...
const fetchData = async () => {
  try {
    const response = await fetch(
      "http://127.0.0.1:8000/api/sekolah/tugas_sekolah/" + route.params.id
    );
    ...
    selectedMatpel.value = {
      name: data.matpel.pelajaran,
      code: data.matpel_id,
    };

    tgl.value = data.tgl;
    tgl_pengumpulan.value = data.tgl_pengumpulan;
    deskripsi.value = data.deskripsi;
    isi_tugas.value = data.isi_tugas;
  } catch (error) {
    toast.add({
      severity: "error",
...
const handleSubmit = async () => {
  ...
  try {
    const response = await fetch(
      "http://localhost:8000/api/sekolah/tugas_sekolah/" + route.params.id,
      {
        method: "PUT",
        headers: {
