<template>
  <div>
    <div class="font-bold text-4xl mx-12 my-6">Tugas Sekolah</div>
    <div class="bg-[#D8EFE9] rounded-lg p-10 m-5">
      <div class="mb-5">
        <NuxtLink to="/tugas/create">
          <Button label="Tugas Baru" severity="info" />
        </NuxtLink>
      </div>
      <DataTable
...
const columns = [
  ...
  { field: "matpel", header: "Mata Pelajaran" },
  { field: "deskripsi", header: "Deskripsi" },
  { field: "isi_tugas", header: "Isi Tugas" },
  { field: "tgl", header: "Tanggal Tugas" },
  { field: "tgl_pengumpulan", header: "Tanggal Pengumpulan" },
...
const edit = (rowData) => {
  ...
  if (selectedData.value.id !== null) {
    navigateTo("/tugas/" + selectedData.value.id);
  }
};
...
const deleteData = (rowData) => {
  ...
  if (selectedData.value.id !== null) {
    confirm.require({
      ...
      rejectLabel: "Cancel",
      acceptLabel: "Delete",
      rejectClass: "bg-white text-gray-800 border-slate-700 hover:bg-white  ",
      acceptClass:
...
const fetchData = async () => {
  try {
    const response = await fetch(
      "http://127.0.0.1:8000/api/sekolah/tugas_sekolah"
    );
    ...
    dataTable.value = data.map((item, i) => ({
      ...
      guru: item.guru.nama,
      matpel: item.matpel.pelajaran,
      tgl: item.tgl,
      tgl_pengumpulan: item.tgl_pengumpulan,
      deskripsi: item.deskripsi,
      isi_tugas: item.isi_tugas,
    }));
  } catch (error) {
