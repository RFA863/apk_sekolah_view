<template>
  <div>
    <div class="font-bold text-4xl mx-12 my-6">Catatan</div>
    <div class="bg-[#D8EFE9] rounded-lg p-10 m-5">
      <div class="mb-5">
        <NuxtLink to="/catatan/create">
          <Button label="Catatan Baru" severity="info" />
        </NuxtLink>
      </div>
      <DataTable
...
const edit = (rowData) => {
  ...
  if (selectedData.value.id !== null) {
    navigateTo("/catatan/" + selectedData.value.id);
  }
};
...
const fetchData = async () => {
  try {
    const response = await fetch("http://127.0.0.1:8000/api/sekolah/note");
...
const deleteTableData = async () => {
  try {
    const response = await fetch(
      "http://localhost:8000/api/sekolah/note/" + selectedData.value.id,
      {
        method: "DELETE",
        headers: {
