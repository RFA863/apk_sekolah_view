<template>
  <div>
    <div class="font-bold text-4xl mx-12 my-6">Update Catatan</div>
...
const fetchData = async () => {
  try {
    const response = await fetch(
      "http://127.0.0.1:8000/api/sekolah/note/" + route.params.id
    );
...
const handleSubmit = async () => {
  ...
  try {
    const response = await fetch(
      "http://localhost:8000/api/sekolah/note/" + route.params.id,
      {
        method: "PUT",
        headers: {
